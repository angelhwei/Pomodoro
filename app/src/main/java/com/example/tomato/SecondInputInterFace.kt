...
class SecondInputInterFace : AppCompatActivity() {
    var Text_View: TextView? = null
    var Edit_Text: EditText? = null
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_second_input_inter_face)
    ...
    fun Save(v: View?) {
        ...
        if ("" == Edit_Text!!.text.toString().trim { it <= ' ' }) Toast.makeText(
            ...
            it_Record.putExtra("內容2", Edit_Text!!.text.toString())
            it_Record.putExtra("次數2", 2)
            it_Record.putExtra("restNumber", restNumber)
            setResult(Activity.RESULT_OK, it_Record)
            startActivityForResult(it_Record, 1)
            finish()
            val TeleportToInput: Intent
            TeleportToInput = Intent(this, FirstInputInterFace::class.java)
        }
    }
}